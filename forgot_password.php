...
<head>
  ...
  <title>Forgot Password — VidGenius</title>
  ...
  <style>
    ...
    .auth-card { background:#fff;border:1px solid var(--border-c);border-radius:1.5rem;box-shadow:0 24px 60px rgba(37,99,235,.12),0 8px 24px rgba(0,0,0,.07);padding:2.5rem;width:100%;max-width:460px;animation:cardIn .45s cubic-bezier(.22,1,.36,1) both; }
    ...
    .code-input { width:48px;height:56px;text-align:center;font-size:1.5rem;font-weight:700;border:1.5px solid var(--border-c);border-radius:.875rem;font-family:var(--fb);color:#0f172a;outline:none;transition:border-color .15s,box-shadow .15s; }
  ...
  <div style="width:100%;max-width:460px;padding:0 1rem;">
    ...
    <div class="auth-card" id="stepEmail">
      <div class="text-center mb-4">
        ...
        <p class="card-sub mt-1">Enter your email and we'll send you a reset code</p>
      </div>

      <div class="vg-alert mb-3" id="alertEmail" role="alert">
        <svg id="alertEmailIcon" width="17" height="17" fill="none" stroke="currentColor" viewBox="0 0 24 24"></svg>
        <span id="alertEmailText"></span>
      </div>
      ...
      <form id="emailForm" novalidate>
        ...
        <button type="submit" class="btn-vg" id="emailBtn">
          <span class="spin-sm" id="emailSpinner"></span>
    ...
    <!-- STEP 2: Code -->
    <div class="auth-card" id="stepCode" style="display:none">
      <div class="text-center mb-4">
        <div class="card-icon">
          <svg width="26" height="26" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <p class="card-sub mt-1">Enter the 6-digit code we sent to <strong id="emailShown"></strong></p>
      </div>
      ...
      <p class="text-center mt-3" style="font-size:.85rem;color:var(--muted)">
        Didn't receive it? <a href="#" onclick="showStep('email')" style="color:var(--blue);font-weight:600">Try again</a>
      </p>
    </div>

    <!-- STEP 3: New password -->
    <div class="auth-card" id="stepReset" style="display:none">
      <div class="text-center mb-4">
        <div class="card-icon">
          <svg width="26" height="26" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <form id="resetForm" novalidate>
        <div class="mb-3">
          ...
          <div class="iw">
            ...
            <input type="password" class="form-control" id="newPassword" placeholder="Min. 8 characters" minlength="8" required/>
          </div>
        </div>
        <div class="mb-3">
...
<script>
  ...
  function hideAlert(id) { document.getElementById('alert' + id).className = 'vg-alert mb-3'; }

  // ── Steps ──────────────────────────────────────────────
  function showStep(step) {
    document.getElementById('stepEmail').style.display = step === 'email' ? 'block' : 'none';
    document.getElementById('stepCode').style.display  = step === 'code'  ? 'block' : 'none';
    document.getElementById('stepReset').style.display = step === 'reset' ? 'block' : 'none';
  }
  ...
  document.getElementById('emailForm').addEventListener('submit', async (e) => {
    ...
    btn.disabled = true;
    document.getElementById('emailSpinner').classList.add('on');
    document.getElementById('emailBtnText').textContent = 'Sending…';

    try {
      const res  = await fetch(`${API_URL}/app/api/auth/forgot_password.php`, {
        method: 'POST',
      ...
      showAlert('Email', 'error', err.message);
    }

    btn.disabled = false;
    document.getElementById('emailSpinner').classList.remove('on');
    document.getElementById('emailBtnText').textContent = 'Send reset code';
  });
  ...
  async function verifyCode() {
    ...
    if (code.length < 6) { showAlert('Code', 'error', 'Please enter the complete 6-digit code.'); return; }
    ...
    try {
      ...
      showAlert('Code', 'error', err.message || 'Invalid code.');
    }
  ...
  document.getElementById('resetForm').addEventListener('submit', async (e) => {
    ...
    if (pwd.length < 8) { showAlert('Reset', 'error', 'Password must be at least 8 characters.'); return; }
    if (pwd !== confirm) { showAlert('Reset', 'error', 'Passwords do not match.'); return; }
    ...
    try {
      ...
      showAlert('Reset', 'success', 'Password reset successfully! Redirecting to login…');
      setTimeout(() => { window.location.href = '/video/login.php'; }, 2000);

    } catch (err) {
      showAlert('Reset', 'error', err.message || 'An error occurred.');
    }
