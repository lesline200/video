...
.plat-connect-card.connected { border-color: #86efac; background: #f0fdf4; }
.plat-connect-card.selected-not-connected { border-color: #fca5a5; background: #fff5f5; }
...
.plat-connect-info { flex: 1; min-width: 0; }
.plat-connect-name { font-weight: 700; font-size: .95rem; color: var(--txt-1); }
.plat-connect-desc { font-size: .78rem; color: var(--txt-2); margin-top: .1rem; }
.plat-connect-right { display: flex; align-items: center; gap: .75rem; flex-shrink: 0; }

.plat-conn-status {
  display: flex; align-items: center; gap: .35rem;
  font-size: .72rem; font-weight: 700;
  padding: .3rem .75rem; border-radius: var(--r-full); white-space: nowrap;
}
.plat-conn-status.ok { background: var(--green-l); color: var(--green); }
.plat-conn-status.ko { background: #f1f5f9; color: var(--txt-3); }
...
.plat-connect-body.open { display: block; }

.plat-method-grid { display: grid; grid-template-columns: 1fr 1fr; gap: .75rem; margin-bottom: .75rem; }
.plat-method-btn {
  border: 1.5px solid var(--border); border-radius: .75rem;
  background: var(--surface-2); padding: .875rem;
  cursor: pointer; text-align: center;
  transition: all .15s; display: flex; flex-direction: column; align-items: center; gap: .4rem;
}
.plat-method-btn:hover, .plat-method-btn.selected { border-color: var(--blue); background: var(--blue-l); }
.plat-method-icon { width: 34px; height: 34px; border-radius: .5rem; display: flex; align-items: center; justify-content: center; font-size: 1rem; }
.plat-method-label { font-size: .8rem; font-weight: 700; color: var(--txt-1); }
.plat-method-sub   { font-size: .7rem; color: var(--txt-2); }
...
.plat-oauth-btn:hover { background: var(--surface-2); border-color: #93c5fd; }

.plat-notice {
  display: flex; align-items: flex-start; gap: .5rem;
  padding: .625rem .875rem; border-radius: var(--r);
  font-size: .78rem; line-height: 1.5; margin-top: .75rem;
}
.plat-notice.info { background: var(--blue-l); border: 1px solid #bfdbfe; color: #1e40af; }
.plat-notice.warn { background: #fffbeb; border: 1px solid #fde68a; color: #92400e; }

.plat-connected-box {
  display: flex; align-items: center; gap: .875rem;
  padding: .875rem; background: #f0fdf4;
  border: 1px solid #86efac; border-radius: .75rem;
}
.plat-connected-avatar {
  width: 40px; height: 40px; border-radius: 50%;
  background: linear-gradient(135deg,#16a34a,#059669);
  display: flex; align-items: center; justify-content: center;
  color: #fff; font-weight: 800; font-size: 1rem; flex-shrink: 0;
}
.plat-connected-handle { font-weight: 700; font-size: .9rem; color: #166534; }
.plat-connected-sub    { font-size: .75rem; color: #15803d; margin-top: .1rem; }

.plat-err-msg {
  display: none; margin-top: .5rem;
  padding: .5rem .75rem; background: var(--red-l);
...
.plat-block-warn {
  ...
  border-radius: var(--r); font-size: .8rem; color: #92400e;
}
...
<div class="step-panel" id="step-3">
  <h2 class="step-title">Connect Your Platforms</h2>
  <p class="step-sub">Link your accounts to enable auto-posting. At least one platform must be connected to continue.</p>

  <!-- YOUTUBE -->
  <div class="plat-connect-card" id="card-youtube">
    <div class="plat-connect-header" onclick="togglePlatBody('youtube')">
      <div class="plat-connect-logo" style="background:#ef4444">
      ...
      <div class="plat-connect-info">
        <div class="plat-connect-name">YouTube Shorts</div>
        <div class="plat-connect-desc">Auto-post as Shorts to your channel</div>
      </div>
      <div class="plat-connect-right">
        <span class="plat-conn-status ko" id="status-youtube">
          <svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor"><circle cx="4" cy="4" r="4"/></svg>
          Not connected
        </span>
        <label class="toggle" onclick="event.stopPropagation()" style="width:44px;height:24px">
          <input type="checkbox" id="toggle-youtube" onchange="onPlatToggle('youtube',this)">
          <span class="toggle-track"></span>
        </label>
      </div>
    </div>
    <div class="plat-connect-body" id="body-youtube">
      <div id="form-youtube">
        <div class="plat-method-grid">
          <div class="plat-method-btn selected" id="meth-youtube-oauth" onclick="selectPlatMethod('youtube','oauth')">
            <div class="plat-method-icon" style="background:#fef3c7">🔐</div>
            <div class="plat-method-label">Google OAuth</div>
            <div class="plat-method-sub">Recommended</div>
          </div>
          <div class="plat-method-btn" id="meth-youtube-api" onclick="selectPlatMethod('youtube','api')">
            <div class="plat-method-icon" style="background:#ede9fe">🔑</div>
            <div class="plat-method-label">API Key</div>
            <div class="plat-method-sub">Advanced</div>
          </div>
        </div>
        <div id="fields-youtube-oauth">
          <button type="button" class="plat-oauth-btn" onclick="connectOAuth('youtube')">
          ...
          <div class="plat-notice info">
            ...
            You'll be redirected to Google to authorize VidGenius. We only request upload permissions.
          </div>
        </div>
        <div id="fields-youtube-api" style="display:none">
          <div class="step-fields" style="margin-top:1rem">
            <div class="field"><label>YouTube API Key</label><input class="input" type="password" id="yt-api-key" placeholder="AIzaSy..."/></div>
            <div class="field"><label>Channel ID</label><input class="input" type="text" id="yt-channel" placeholder="UCxxxxxxxxxxxxxxxx"/></div>
          </div>
          <button type="button" class="btn btn-primary" style="margin-top:1rem;width:100%;justify-content:center" onclick="connectAPI('youtube')">
          ...
          <p class="plat-err-msg" id="err-youtube">Please fill in all fields.</p>
        </div>
      </div>
      <div id="connected-youtube" style="display:none">
        <div class="plat-connected-box">
          <div class="plat-connected-avatar" id="yt-avatar">Y</div>
          <div>
            <div class="plat-connected-handle" id="yt-handle">@channel</div>
            <div class="plat-connected-sub" id="yt-sub">YouTube channel connected</div>
          </div>
          <svg width="20" height="20" fill="none" stroke="#16a34a" viewBox="0 0 24 24" style="margin-left:auto"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
        </div>
        <button type="button" class="btn btn-danger" style="width:100%;justify-content:center;margin-top:.75rem" onclick="disconnectPlat('youtube')">Disconnect YouTube</button>
      </div>
    </div>
  </div>

  <!-- TIKTOK -->
  <div class="plat-connect-card" id="card-tiktok">
    <div class="plat-connect-header" onclick="togglePlatBody('tiktok')">
      <div class="plat-connect-logo" style="background:#000">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.34 6.34 0 00-.79-.05A6.34 6.34 0 003.15 15.3a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.72a8.16 8.16 0 004.77 1.52v-3.5a4.83 4.83 0 01-1.01-.05z"/></svg>
      </div>
      <div class="plat-connect-info">
        <div class="plat-connect-name">TikTok</div>
        <div class="plat-connect-desc">Auto-post videos to your TikTok</div>
      </div>
      <div class="plat-connect-right">
        <span class="plat-conn-status ko" id="status-tiktok">
          <svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor"><circle cx="4" cy="4" r="4"/></svg>
          Not connected
        </span>
        <label class="toggle" onclick="event.stopPropagation()" style="width:44px;height:24px">
          <input type="checkbox" id="toggle-tiktok" onchange="onPlatToggle('tiktok',this)">
          <span class="toggle-track"></span>
        </label>
      </div>
    </div>
    <div class="plat-connect-body" id="body-tiktok">
      <div id="form-tiktok">
        <div class="plat-notice info" style="margin-bottom:1rem">
          <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          Connect via TikTok's official login. We request only video upload permissions.
        </div>
        <button type="button" class="plat-oauth-btn" onclick="connectOAuth('tiktok')" style="background:#111;color:#fff;border-color:#333">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.34 6.34 0 00-.79-.05A6.34 6.34 0 003.15 15.3a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.72a8.16 8.16 0 004.77 1.52v-3.5a4.83 4.83 0 01-1.01-.05z"/></svg>
          Connect with TikTok
        </button>
        <div class="plat-method-grid" style="margin-top:.875rem">
          <div class="plat-method-btn" id="meth-tiktok-api" onclick="selectPlatMethod('tiktok','api')">
            <div class="plat-method-icon" style="background:#fef3c7">🔑</div>
            <div class="plat-method-label">Access Token</div>
            <div class="plat-method-sub">Manual setup</div>
          </div>
        </div>
        <div id="fields-tiktok-api" style="display:none">
          <div class="step-fields" style="margin-top:1rem">
            <div class="field"><label>TikTok Access Token</label><input class="input" type="password" id="tt-token" placeholder="Paste your access token"/></div>
            <div class="field"><label>Open ID</label><input class="input" type="text" id="tt-openid" placeholder="Paste your open ID"/></div>
          </div>
          <button type="button" class="btn btn-primary" style="margin-top:1rem;width:100%;justify-content:center" onclick="connectAPI('tiktok')">Connect</button>
          <p class="plat-err-msg" id="err-tiktok">Please fill in all fields.</p>
        </div>
      </div>
      <div id="connected-tiktok" style="display:none">
        <div class="plat-connected-box">
          <div class="plat-connected-avatar" id="tt-avatar" style="background:#111">T</div>
          <div>
            <div class="plat-connected-handle" id="tt-handle">@account</div>
            <div class="plat-connected-sub" id="tt-sub">TikTok account connected</div>
          </div>
          <svg width="20" height="20" fill="none" stroke="#16a34a" viewBox="0 0 24 24" style="margin-left:auto"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
        </div>
        <button type="button" class="btn btn-danger" style="width:100%;justify-content:center;margin-top:.75rem" onclick="disconnectPlat('tiktok')">Disconnect TikTok</button>
      </div>
    </div>
  </div>

  <!-- INSTAGRAM -->
  <div class="plat-connect-card" id="card-instagram">
    <div class="plat-connect-header" onclick="togglePlatBody('instagram')">
      <div class="plat-connect-logo" style="background:linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)">
      ...
      <div class="plat-connect-info">
        <div class="plat-connect-name">Instagram Reels</div>
        <div class="plat-connect-desc">Auto-post as Reels via Meta Business</div>
      </div>
      <div class="plat-connect-right">
        <span class="plat-conn-status ko" id="status-instagram">
          <svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor"><circle cx="4" cy="4" r="4"/></svg>
          Not connected
        </span>
        <label class="toggle" onclick="event.stopPropagation()" style="width:44px;height:24px">
          <input type="checkbox" id="toggle-instagram" onchange="onPlatToggle('instagram',this)">
          <span class="toggle-track"></span>
        </label>
      </div>
    </div>
    <div class="plat-connect-body" id="body-instagram">
      <div id="form-instagram">
        <div class="plat-notice warn" style="margin-bottom:1rem">
          ...
          Requires a <strong>Professional Instagram account</strong> linked to a Facebook Page via Meta Business Suite.
        </div>
        <button type="button" class="plat-oauth-btn" onclick="connectOAuth('instagram')" style="background:linear-gradient(135deg,#f09433,#dc2743);color:#fff;border:none">
          Connect via Meta
        </button>
        <div class="plat-method-grid" style="margin-top:.875rem">
          <div class="plat-method-btn" id="meth-instagram-api" onclick="selectPlatMethod('instagram','api')">
            <div class="plat-method-icon" style="background:#fce7f3">🔑</div>
            <div class="plat-method-label">Access Token</div>
            <div class="plat-method-sub">Manual setup</div>
          </div>
        </div>
        <div id="fields-instagram-api" style="display:none">
          <div class="step-fields" style="margin-top:1rem">
            <div class="field"><label>Instagram Access Token</label><input class="input" type="password" id="ig-token" placeholder="Paste long-lived access token"/></div>
            <div class="field"><label>Business Account ID</label><input class="input" type="text" id="ig-account" placeholder="1234567890"/></div>
          </div>
          <button type="button" class="btn btn-primary" style="margin-top:1rem;width:100%;justify-content:center" onclick="connectAPI('instagram')">Connect</button>
          <p class="plat-err-msg" id="err-instagram">Please fill in all fields.</p>
        </div>
      </div>
      <div id="connected-instagram" style="display:none">
        <div class="plat-connected-box">
          <div class="plat-connected-avatar" id="ig-avatar" style="background:linear-gradient(135deg,#f09433,#dc2743)">I</div>
          <div>
            <div class="plat-connected-handle" id="ig-handle">@account</div>
            <div class="plat-connected-sub" id="ig-sub">Instagram account connected</div>
          </div>
          <svg width="20" height="20" fill="none" stroke="#16a34a" viewBox="0 0 24 24" style="margin-left:auto"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
        </div>
        <button type="button" class="btn btn-danger" style="width:100%;justify-content:center;margin-top:.75rem" onclick="disconnectPlat('instagram')">Disconnect Instagram</button>
      </div>
    </div>
  </div>

  <!-- Avertissement bloquant -->
  <div class="plat-block-warn" id="plat-block-warning" style="display:none">
    ...
    <span id="plat-block-msg">Connect at least one platform to continue.</span>
  </div>

  <p style="text-align:center;font-size:.78rem;color:var(--txt-3);margin-top:1rem;padding-top:1rem;border-top:1px solid var(--border)">
    <strong style="color:var(--red)">*</strong> At least one platform must be connected to continue.
  </p>
</div>
    ...
    <div class="field">
      ...
      <input class="input" type="password" id="settingPwd" placeholder="Leave empty to keep current"/>
        ...
        <div class="settings-section">
          ...
          <div class="settings-fields">
<!-- Notifications section — remplace les toggles existants -->
...
<div class="toggle-row">
  <div class="toggle-info">
    <p>Weekly reports</p>
    <p>Receive a weekly summary of your channel performance</p>
  </div>
  <label class="toggle">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  f1.href = "https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,400;12..96,500;12..96,600;12..96,700;12..96,800&family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,400;0,9..40,500;0,9..40,600&display=swap";
  f1.rel = "stylesheet";
  document.head.appendChild(f1);
  const f2 = document.createElement('link');
  f2.href = "https://fonts.googleapis.com"; f2.rel = "preconnect";
  document.head.appendChild(f2);
  const f3 = document.createElement('link');
  f3.href = "https://fonts.gstatic.com"; f3.rel = "preconnect";
...
function showWizardError(msg) {
  let bar = document.getElementById('wizardErrorBar');
  if (!bar) {
    bar = document.createElement('div');
    bar.id = 'wizardErrorBar';
    bar.style.cssText = `display:flex;align-items:center;gap:.625rem;padding:.75rem 1rem;margin-top:1.25rem;background:#fee2e2;border:1px solid #fca5a5;border-radius:.875rem;font-size:.82rem;color:#dc2626;animation:pageIn .2s ease;`;
    bar.innerHTML = `<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="flex-shrink:0"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg><span id="wizardErrorText"></span>`;
...
function clearWizardError() {
  const bar = document.getElementById('wizardErrorBar');
...
function highlightField(fieldId, hasError) {
  ...
  if (hasError) {
    ...
    el.style.boxShadow   = '0 0 0 4px rgba(220,38,38,.1)';
    el.addEventListener('input',  () => { highlightField(fieldId, false); clearWizardError(); }, { once: true });
    el.addEventListener('change', () => { highlightField(fieldId, false); clearWizardError(); }, { once: true });
  } else {
...
function wizardNext() {
  if (wizardStep === 1) {
    ...
    const niche = document.getElementById('seriesNiche').value;
    const errors = [];
    if (!name)  errors.push('Series Name is required.');
    if (!niche) errors.push('Please select a Niche.');
    if (errors.length > 0) { showWizardError(errors.join(' ')); highlightField('seriesName',!name); highlightField('seriesNiche',!niche); return; }
    clearWizardError(); highlightField('seriesName',false); highlightField('seriesNiche',false);
  }
  if (wizardStep === 2) {
    ...
    if (!style)  errors.push('Please select an Image Style.');
    if (!length) errors.push('Please select a Video Length.');
    if (errors.length > 0) { showWizardError(errors.join(' ')); highlightField('voiceSelect',!voice); highlightField('imageStyle',!style); highlightField('videoLength',!length); return; }
    clearWizardError(); highlightField('voiceSelect',false); highlightField('imageStyle',false); highlightField('videoLength',false);
  }
  if (wizardStep === 3) {
    const check = platStepIsValid();
    if (!check.valid) {
      showWizardError(check.msg);
      const warn = document.getElementById('plat-block-warning');
      const msg  = document.getElementById('plat-block-msg');
      if (warn && msg) { msg.textContent = check.msg; warn.style.display = 'flex'; }
      document.querySelector('.wizard-wrap').scrollIntoView({ behavior:'smooth', block:'end' });
      return;
    }
    clearWizardError();
  }
  if (wizardStep === 4) {
    const cta  = document.getElementById('callToAction').value.trim();
    const tone = document.getElementById('toneSelect').value;
    const errors = [];
    if (!cta)  errors.push('Call to Action is required.');
    if (!tone) errors.push('Please select a Tone.');
    if (errors.length > 0) { showWizardError(errors.join(' ')); highlightField('callToAction',!cta); highlightField('toneSelect',!tone); return; }
    clearWizardError(); highlightField('callToAction',false); highlightField('toneSelect',false);
  }
  if (wizardStep === 5) {
    const caption = document.getElementById('captionStyle').value;
    if (!caption) { showWizardError('Please select a Caption Style.'); return; }
    clearWizardError();
  }
  if (wizardStep === 6) {
    ...
    const errors = [];
    if (activeDays.length === 0) errors.push('Please select at least one posting day.');
    if (!time)     errors.push('Please set a posting time.');
    if (!timezone) errors.push('Please select a timezone.');
    if (errors.length > 0) {
      showWizardError(errors.join(' '));
      const daysGrid = document.getElementById('daysGrid');
      if (activeDays.length === 0 && daysGrid) {
        daysGrid.style.outline = '2px solid #dc2626';
        daysGrid.style.outlineOffset = '4px';
        daysGrid.style.borderRadius  = '.625rem';
        daysGrid.querySelectorAll('.day-btn').forEach(btn => {
          btn.addEventListener('click', () => { daysGrid.style.outline = ''; clearWizardError(); }, { once: true });
        });
      }
      highlightField('scheduleTime',!time); highlightField('timezoneSelect',!timezone);
      return;
    }
    clearWizardError();
    const daysGrid = document.getElementById('daysGrid');
    if (daysGrid) daysGrid.style.outline = '';
    highlightField('scheduleTime',false); highlightField('timezoneSelect',false);
  }
...
function buildReview() {
  const platLabels = { youtube:'YouTube', tiktok:'TikTok', instagram:'Instagram' };
  const connectedPlats = Object.keys(platConnected).filter(p => platConnected[p]).map(p => platLabels[p]).join(', ') || 'None';
  const days = [...document.querySelectorAll('.day-btn.active')].map(b => b.dataset.day.charAt(0).toUpperCase() + b.dataset.day.slice(1,3)).join(', ') || '—';
  const captionRaw   = document.getElementById('captionStyle').value || '—';
  const captionLabel = captionRaw.replace(/-/g,' ').replace(/\b\w/g, l => l.toUpperCase());
  const hashtagsLabel = tags.length > 0 ? tags.map(t => '#'+t).join(', ') : 'None';
  const speedVal = document.getElementById('voiceSpeed')?.value || '1.0';
  const fields = [
    ['Series Name',    document.getElementById('seriesName').value.trim()  || '—'],
    ['Niche',          document.getElementById('seriesNiche').value         || '—'],
    ['Description',    document.getElementById('seriesDesc').value.trim()   || 'None'],
    ['Voice',          document.getElementById('voiceSelect').value         || '—'],
    ['Voice Speed',    speedVal + 'x'],
    ['Image Style',    document.getElementById('imageStyle').value          || '—'],
    ['Video Length',   document.getElementById('videoLength').value + 's'  || '—'],
    ['Platforms',      connectedPlats],
    ['Caption Style',  captionLabel],
    ['Tone',           document.getElementById('toneSelect').value          || '—'],
    ['Call to Action', document.getElementById('callToAction').value.trim() || '—'],
    ['Hashtags',       hashtagsLabel],
    ['Posting Days',   days],
    ['Posting Time',   document.getElementById('scheduleTime').value        || '—'],
    ['Timezone',       document.getElementById('timezoneSelect').value      || '—'],
  ];
  document.getElementById('reviewGrid').innerHTML = fields.map(([label,value]) =>
    `<div class="review-item"><label>${label}</label><p>${value}</p></div>`
  ).join('');
}
...
function showToast(msg, type = 'success') {
  document.getElementById('vg-toast')?.remove();
  const colors = {
    success: { bg:'var(--green-l)', border:'#86efac', color:'var(--green)', icon:'✓' },
    error:   { bg:'var(--red-l)',   border:'#fca5a5', color:'var(--red)',   icon:'✕' },
    info:    { bg:'var(--blue-l)',  border:'#93c5fd', color:'var(--blue)',  icon:'ℹ' },
  };
  const c = colors[type] || colors.info;
  const toast = document.createElement('div');
  toast.id = 'vg-toast';
  toast.style.cssText = `position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;display:flex;align-items:center;gap:.625rem;padding:.875rem 1.25rem;background:${c.bg};border:1.5px solid ${c.border};border-radius:.875rem;color:${c.color};font-size:.875rem;font-weight:600;box-shadow:0 8px 24px rgba(0,0,0,.12);animation:toastIn .25s cubic-bezier(.22,1,.36,1);max-width:360px;`;
  ...
  toast.innerHTML = `<span style="font-size:1rem;flex-shrink:0">${c.icon}</span><span>${msg}</span><button onclick="this.parentNode.remove()" style="margin-left:auto;background:none;border:none;cursor:pointer;color:inherit;font-size:1rem;line-height:1;opacity:.6">✕</button>`;
...
function validateSettingsForm() {
  ...
  const currentPwd = document.getElementById('settingCurrentPwd')?.value;
  const errors     = [];
  if (!name || name.length < 2) {
    errors.push('Name must be at least 2 characters.');
    highlightSettingField('settingName', true);
  } else {
    highlightSettingField('settingName', false);
  }
  if (newPwd) {
    if (!currentPwd) { errors.push('Current password required to set a new one.'); highlightSettingField('settingCurrentPwd', true); }
    else { highlightSettingField('settingCurrentPwd', false); }
    if (newPwd.length < 8) { errors.push('New password must be at least 8 characters.'); highlightSettingField('settingPwd', true); }
    else { highlightSettingField('settingPwd', false); }
    if (currentPwd && newPwd === currentPwd) { errors.push('New password must differ from current password.'); highlightSettingField('settingPwd', true); }
  }
  return errors;
}
function highlightSettingField(fieldId, hasError) {
  const el = document.getElementById(fieldId);
  if (!el) return;
  if (hasError) {
    el.style.borderColor = '#dc2626';
    el.style.boxShadow   = '0 0 0 4px rgba(220,38,38,.1)';
    const clear = () => highlightSettingField(fieldId, false);
...
async function saveSettings() {
  ...
  const body = { name };
  if (newPwd) { body.new_password = newPwd; body.current_password = currentPwd; }

  try {

  
    const res  = await fetch('/video/app/api/user/update_profile.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body), credentials:'include' });
    const text = await res.text();
    let data;
    try { data = JSON.parse(text); } catch { throw new Error('Server returned an unexpected response. Check PHP logs.'); }
    ...
    if (settingNameEl) settingNameEl.value     = data.name;
    const pwdEl    = document.getElementById('settingPwd');
    const currPwdEl = document.getElementById('settingCurrentPwd');
    if (pwdEl)    pwdEl.value    = '';
    if (currPwdEl) currPwdEl.value = '';
    btn.innerHTML = `<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg> Saved!`;
...
async function deleteAccount() {
  const confirmed = confirm('⚠️ Delete account?\n\nThis will permanently delete your account, all series, and all videos. This action CANNOT be undone.');
  if (!confirmed) return;
  const password = prompt('Enter your password to confirm deletion:');
  if (password === null || password.trim() === '') { showToast('Password is required to delete your account.', 'error'); return; }
  try {
    const res  = await fetch('/video/app/api/user/delete_account.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ password }), credentials:'include' });
    const text = await res.text();
    let data;
    ...
    setTimeout(() => { window.location.href = '/video/index.php'; }, 2000);
    ...
    return `
      <div class="card series-card" id="series-${s.id}">
        <div class="series-card__head">
          <div><p class="series-card__name">${s.name}</p><p class="series-card__niche">${s.niche}</p></div>
        ...
        <div class="series-card__footer">
          ...
          <div style="display:flex;gap:.5rem">
            ...
            <button class="btn btn-danger" style="padding:.4rem .875rem;font-size:.78rem" onclick="deleteSeries('${s.id}','${s.name}')">Delete</button>
          </div>
        </div>
      </div>`;
  }).join('');
...
async function toggleSeries(seriesId) {
  try {
    const res  = await fetch('/video/app/api/series/update.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'toggle',series_id:seriesId}), credentials:'include' });
    const data = await safeJson(res);
    if (!data.success) throw new Error(data.error);
    loadSeries();
  } catch (err) { showToast(err.message || 'Error updating series.', 'error'); }
}
async function deleteSeries(seriesId, seriesName) {
  if (!confirm(`Delete "${seriesName}"? This cannot be undone.`)) return;
  try {
    const res  = await fetch('/video/app/api/series/update.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'delete',series_id:seriesId}), credentials:'include' });
    const data = await safeJson(res);
    if (!data.success) throw new Error(data.error);
    loadSeries();
  } catch (err) { showToast(err.message || 'Error deleting series.', 'error'); }
}
...
async function submitSeries() {
  ...
  const platforms = [];
 if (document.getElementById('toggle-youtube')?.checked   && platConnected.youtube)   platforms.push('youtube');
 if (document.getElementById('toggle-tiktok')?.checked    && platConnected.tiktok)    platforms.push('tiktok');
 if (document.getElementById('toggle-instagram')?.checked && platConnected.instagram) platforms.push('instagram');
  ...
  const body = {
    ...
    generate_initial_video: true, // Générer une vidéo initiale
  };
  if (!body.name)  { showToast('Series name is required.','error'); btn.disabled=false; btn.innerHTML='Create Series'; return; }
  if (!body.niche) { showToast('Please select a niche.','error');   btn.disabled=false; btn.innerHTML='Create Series'; return; }
  try {
    const res  = await fetch('/video/app/api/series/create.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body), credentials:'include' });
    const data = await safeJson(res);
...
async function loadVideos() {
  try {
    ...
    if (sel) {
      sel.innerHTML = '<option value="all">All Series</option>' + seriesSet.map(s => `<option value="${s}">${s}</option>`).join('');
    }
    filterVideos();
    ...
    return `
      <div class="card vcard" onclick="openVideoDetail('${v.id}')">
        <div class="vcard-thumb">
          ${thumb ? `<img src="${thumb}" alt="" loading="lazy" onerror="this.style.display='none'" style="width:100%;height:100%;object-fit:cover;object-position:center top;display:block"/>` : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;"><svg width="32" height="32" fill="none" stroke="rgba(148,163,184,.4)" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg></div>`}
          <div class="vcard-overlay">
            <div class="vcard-top"><span class="vcard-status-dot" style="background:${dot}"></span></div>
          ...
          ${v.status === 'completed' && v.video_url ? `<div class="vcard-play"><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></div>` : ''}
        </div>
...
function openVideoDetail(videoId) {
  ...
  if (saveBtn) {
    ...
    saveBtn.innerHTML = `<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg> All changes saved`;
  }
  document.getElementById('videosListView').style.display   = 'none';
  document.getElementById('videosDetailView').style.display = 'block';
  window.scrollTo({ top:0, behavior:'smooth' });
}
function closeVideoDetail() {
  const video = document.getElementById('detailVideo');
  if (video) { video.pause(); video.src = ''; }
  document.getElementById('videosDetailView').style.display = 'none';
...
async function deleteCurrentVideo() {
  ...
  try {
    const res  = await fetch('/video/app/api/videos/delete.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({video_id:currentVideoId}), credentials:'include' });
    const data = await safeJson(res);
    if (!data.success) throw new Error(data.error);
    closeVideoDetail(); loadVideos(); loadSeries();
  } catch(err) { showToast(err.message || 'Error deleting video.', 'error'); }
}
async function deleteVideo(videoId) {
  if (!confirm('Delete this video? This cannot be undone.')) return;
  try {
    const res  = await fetch('/video/app/api/videos/delete.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({video_id:videoId}), credentials:'include' });
    const data = await safeJson(res);
    if (!data.success) throw new Error(data.error);
  ...
  try {
    const res = await fetch('/video/app/api/videos/update.php', {
      ...
      body: JSON.stringify({
        ...
        title:       document.getElementById('vdTitle').value,
        description: document.getElementById('vdDesc').value,
        sched_date:  document.getElementById('vdSchedDate').value,
        sched_time:  document.getElementById('vdSchedTime').value,
        published:   document.getElementById('vdPublished').checked,
      }),
      credentials: 'include'
    });
    ...
    setTimeout(() => {
      ...
      btn.innerHTML = `<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg> All changes saved`;
    }, 2500);
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { closeVideoModal(); if (currentVideoId) closeVideoDetail(); }
});

/* ══════════════════════════════════════
   PLATFORM CONNECT — Step 3
══════════════════════════════════════ */
const platConnected = { youtube: false, tiktok: false, instagram: false };
const platFakeAccounts = {
  youtube:   { handle:'@YourChannel', sub:'YouTube channel connected',   avatar:'Y' },
  tiktok:    { handle:'@yourtiktok',  sub:'TikTok account connected',    avatar:'T' },
  instagram: { handle:'@yourinsta',   sub:'Instagram account connected', avatar:'I' },
};
function togglePlatBody(plat) {
  document.getElementById('body-' + plat).classList.toggle('open');
}
function onPlatToggle(plat, el) {
  if (el.checked && !platConnected[plat]) document.getElementById('body-' + plat).classList.add('open');
  updatePlatCard(plat);
}
function updatePlatCard(plat) {
  const tog  = document.getElementById('toggle-' + plat);
  const card = document.getElementById('card-' + plat);
  const warn = document.getElementById('plat-block-warning');
  const msg  = document.getElementById('plat-block-msg');
  card.classList.remove('connected','selected-not-connected');
  if (platConnected[plat]) {
    card.classList.add('connected');
  } else if (tog && tog.checked) {
    card.classList.add('selected-not-connected');
    if (warn && msg) {
      msg.textContent = plat.charAt(0).toUpperCase()+plat.slice(1) + ' is enabled but not connected. Please connect it or disable the toggle.';
...
function selectPlatMethod(plat, method) {
  ['oauth','api'].forEach(m => {
    const btn    = document.getElementById('meth-' + plat + '-' + m);
    const fields = document.getElementById('fields-' + plat + '-' + m);
    if (btn)    btn.classList.toggle('selected', m === method);
    if (fields) fields.style.display = (m === method) ? 'block' : 'none';
  });
}
function connectOAuth(plat) {
  const btn  = event.target.closest('button');
  const orig = btn.innerHTML;
  btn.innerHTML = '<span style="display:inline-block;width:14px;height:14px;border:2px solid rgba(0,0,0,.2);border-top-color:#555;border-radius:50%;animation:spin .7s linear infinite"></span> Connecting…';
  btn.disabled  = true;
  setTimeout(() => { btn.innerHTML = orig; btn.disabled = false; platMarkConnected(plat); }, 1800);
}
function connectAPI(plat) {
  const errEl  = document.getElementById('err-' + plat);
  const checks = { youtube:['yt-api-key','yt-channel'], tiktok:['tt-token','tt-openid'], instagram:['ig-token','ig-account'] };
  const valid  = (checks[plat] || []).every(id => { const el = document.getElementById(id); return el && el.value.trim(); });
  if (!valid) {
    if (errEl) { errEl.style.display = 'block'; setTimeout(() => errEl.style.display = 'none', 3000); }
    return;
  }
  platMarkConnected(plat);
}
function platMarkConnected(plat) {
  platConnected[plat] = true;
  const acc    = platFakeAccounts[plat];
  const form   = document.getElementById('form-' + plat);
  const box    = document.getElementById('connected-' + plat);
  const status = document.getElementById('status-' + plat);
  if (form) form.style.display = 'none';
  if (box)  box.style.display  = 'block';
  const ids = { youtube:['yt-handle','yt-sub','yt-avatar'], tiktok:['tt-handle','tt-sub','tt-avatar'], instagram:['ig-handle','ig-sub','ig-avatar'] }[plat] || [];
  if (ids[0]) { const el = document.getElementById(ids[0]); if (el) el.textContent = acc.handle; }
  if (ids[1]) { const el = document.getElementById(ids[1]); if (el) el.textContent = acc.sub;    }
  if (ids[2]) { const el = document.getElementById(ids[2]); if (el) el.textContent = acc.avatar; }
  if (status) { status.className = 'plat-conn-status ok'; status.innerHTML = '<svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor"><circle cx="4" cy="4" r="4"/></svg> Connected'; }
  const tog = document.getElementById('toggle-' + plat);
  if (tog) tog.checked = true;
  document.getElementById('plat-block-warning').style.display = 'none';
  updatePlatCard(plat);
}

function disconnectPlat(plat) {
  if (!confirm('Disconnect ' + plat.charAt(0).toUpperCase() + plat.slice(1) + '?')) return;
  platConnected[plat] = false;
  const form   = document.getElementById('form-' + plat);
  const box    = document.getElementById('connected-' + plat);
  const status = document.getElementById('status-' + plat);
  const tog    = document.getElementById('toggle-' + plat);
  ...
  if (tog)    tog.checked        = false; // ← déjà présent, bien
  if (status) { status.className = 'plat-conn-status ko'; status.innerHTML = '<svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor"><circle cx="4" cy="4" r="4"/></svg> Not connected'; }
  updatePlatCard(plat);
}

function platStepIsValid() {
  const plats = ['youtube','tiktok','instagram'];
  const enabledNotConnected = plats.find(p => {
    const tog = document.getElementById('toggle-' + p);
    return tog && tog.checked && !platConnected[p];
  });
  if (enabledNotConnected) {
    const name = enabledNotConnected.charAt(0).toUpperCase() + enabledNotConnected.slice(1);
    return { valid:false, msg: name + ' is enabled but not connected. Please connect it or disable the toggle.' };
  }
  const anyConnected = plats.some(p => platConnected[p]);
  if (!anyConnected) return { valid:false, msg:'Please connect at least one platform (YouTube, TikTok or Instagram) to continue.' };
  return { valid:true };
}


// GENERATION DES SCRIPTS
async function generateScript(topic, niche, duration) {
    const res = await fetch('/video/app/api/ai/generate_script.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (json.success) {
        console.log('Titre :', json.data.title);
        console.log('Hook :', json.data.hook);
        console.log('Script :', json.data.script);
        console.log('CTA :', json.data.cta);
    } else {
